...
<TABLE class='table table-striped'>
  ...
  <c:forEach var="categrp_Cate_join" items="${list }">  <!-- request 객체에 접근 -->
    ...
    <TR>
      ...
      <TD class="td_bs_left"><A href="../contents/list?cateno=${cateno }">${name }</A></TD>
      <TD class="td_bs">${rdate.substring(0, 10) }</TD>
      <TD class="td_bs">
        <c:choose>
          <c:when test="${visible == 'Y'}">
            <A href="./update_visible?cateno=${cateno }&categrpno=${categrpno }&visible=${visible }"><IMG src="./images/open.png"></A>
          </c:when>
          <c:otherwise>
            <A href="./update_visible?cateno=${cateno }&categrpno=${categrpno }&visible=${visible }"><IMG src="./images/close.png"></A>
          </c:otherwise>
        </c:choose>
      </TD>
