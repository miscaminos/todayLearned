...
import spring.utility.webtest.*;


//Repository: annotation used for DAO pattern class of database access logic
@Repository
public class BbsDAO {
	
	
	//해당 글 삭제 전에 댓글이 달린 글은 삭제하지 않도록 체크하는 메소드
	//bbsno가 부모글로 등록되어있다면 true값을 반환
	public boolean checkRefnum(int bbsno) {
		//해당 글의 bbsno가 부모글번호(refnum)로 등록된 글이 있다 vs. 없다 표기할 boolean
		boolean flag = false;
	...
	//부모글에 새로 달린 답글 번호를 현재 존재하는 답글 번호보다 1증가하는 메소드
	...
	//답글 추가, 정상 등록 시 true 값을 반환하는 메소드
	public boolean createReply(BbsDTO dto) {
	    //정상적으로 reply등록되는지 check flag
		boolean flag = false;
	...
	//답글을 조회하기위해 bbsno에 해당하는 답글을 DB에서 받아서 DTO 객체로 반환하는 메소드
	 ...
	 //bbsno 번호 글을 정상적으로 삭제되면 true를 반환하는 메소드
	  ...
	  //password 체크 성공 시 true를 반환하는 메소드
	  //매개변수 Map map에 글번호, 비밀번호 담아온다.
	  ...
	  public boolean update(BbsDTO dto) {
	    ...
	    try {
	      //if문이 실행 될때, 안될대 가만하기위해 index를 i로 대채한다. (++i로 일씩 증가.)
			int i = 0;
			pstmt = con.prepareStatement(sql.toString());
