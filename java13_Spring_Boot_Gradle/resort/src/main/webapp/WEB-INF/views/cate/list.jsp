...
<body>
  ...
  <DIV class='title_line'>
    <A href="../categrp/list">카테고리 그룹</A> > ${categrpVO.name }
  </DIV>
  ...
  <TABLE class='table table-striped'>
    ...
    <c:forEach var="cateVO" items="${list }">  <!-- request 객체에 접근 -->
      ...
      <TR>
        ...
        <TD class="td_bs_left"><A href="../contents/list?cateno=${cateno }">${cateVO.name }</A></TD>
        <TD class="td_bs">${cateVO.rdate.substring(0, 10) }</TD>
        <TD class="td_bs">
          <c:choose>
            <c:when test="${cateVO.visible == 'Y'}">
              <A href="./update_visible?cateno=${cateno }&visible=${cateVO.visible }&categrpno=${param.categrpno}"><IMG src="./images/open.png"></A>
            </c:when>
            <c:otherwise>
              <A href="./update_visible?cateno=${cateno }&visible=${cateVO.visible }&categrpno=${param.categrpno}"><IMG src="./images/close.png"></A>
            </c:otherwise>
          </c:choose>
        </TD>
